...
def parse_labels(xlsx):
    ...
    for r in range(1, ws.max_row + 1):
        ...
        vid, nfr, quad = ws.cell(r, 3).value, ws.cell(r, 4).value, ws.cell(r, 6).value
        if vid is not None and split and quad in QMAP and nfr:
            out[split].append((int(nfr), QMAP[quad]))         # (n_frames, label) in spreadsheet order
    return out

def match_folders(split_dir, label_list):
    """Map each hex folder -> label by matching its frame count to the spreadsheet (frame counts are distinct)."""
    folders = sorted(d for d in os.listdir(split_dir) if os.path.isdir(os.path.join(split_dir, d)))
    # count frames per folder
    fcount = {}
    for f in folders:
    ...
    remaining = list(label_list)                              # (n_frames, label)
    mapping = []                                              # (folder, label, matched_nframes)
    used = [False] * len(remaining)
    print(f'  matching {len(folders)} folders to {len(remaining)} labels in {os.path.basename(split_dir)}:')
    for f in folders:
        n = fcount[f]
        # exact frame-count match first, else nearest unused
        best_j, best_d = -1, 1e18
        for j, (nf, lb) in enumerate(remaining):
            if used[j]: continue
            d = abs(nf - n)
            if d < best_d: best_d, best_j = d, j
        nf, lb = remaining[best_j]; used[best_j] = True
        mapping.append((os.path.join(split_dir, f), lb))
        flag = '' if best_d == 0 else f'  <-- INEXACT (folder {n} vs label {nf}, off {best_d})'
        print(f'    {f[:8]}.. frames={n:<6} -> {IDQ[lb]}{flag}')
    return mapping

# =============================== CLIP SAMPLING ===============================
def build_clips(folder_label, stride):
    """Return list of (clip_paths, label, video_id). One video -> many clips (sliding windows)."""
    samples = []
    for vid_id, (folder, label) in enumerate(folder_label):
        idir = os.path.join(folder, 'input')
        frames = sorted(f for f in os.listdir(idir) if f.lower().endswith(IMG_EXTS))
        n = len(frames)
        if n == 0: continue
        starts = list(range(0, max(1, n - CLIP_LEN + 1), stride)) or [0]
        for s in starts:
            idxs = [min(s + d, n - 1) for d in range(CLIP_LEN)]
            samples.append(([os.path.join(idir, frames[k]) for k in idxs], label, vid_id))
    return samples
...
def evaluate(model, loader, n_videos, video_labels, weight=None):
    ...
    for x, y, vid in loader:
        ...
        loss_tot += F.cross_entropy(lo, y, weight=weight.cpu() if weight is not None else None,
                                    reduction='sum').item()
        clip_correct += (lo.argmax(1) == y).sum().item()
        clip_n += x.size(0)
...
def main():
    ...
    tr = build_clips(fl['train'], TRAIN_STRIDE)
    va = build_clips(fl['val'],   EVAL_STRIDE)
    te = build_clips(fl['test'],  EVAL_STRIDE)
    va_labels = [lb for _, lb in fl['val']]
    te_labels = [lb for _, lb in fl['test']]
    print(f'\nclips -> train {len(tr)} | val {len(va)} ({len(fl["val"])} videos) | test {len(te)} ({len(fl["test"])} videos)')
    ...
    for ep in range(NUM_EPOCHS):
        ...
        if v['overall_acc'] > best['val']:
            best.update(val=v['overall_acc'], epoch=ep+1); torch.save(model.state_dict(), ckpt)
        print(f'  ep {ep+1}: loss {log[-1]["train_loss"]:.3f} train_acc {tr_acc:.3f}  '
              f'val_loss {v["loss"]:.3f} val_clip_acc {v["clip_acc"]:.3f}  '
              f'VAL acc {v["overall_acc"]:.3f} bal {v["balanced_acc"]:.3f}  (best {best["val"]:.3f} @ ep{best["epoch"]})')
    ...
    print(SEP)
    rows = [
        ('VAL accuracy (model selection)', f'{best["val"]:.4f}  (ep {best["epoch"]})'),
        ('TEST accuracy (overall)',        f'{test["overall_acc"]:.4f}'),
        ('TEST balanced accuracy',         f'{test["balanced_acc"]:.4f}'),
    ...
    res = {'model': MODEL, 'regime': REGIME, 'n_classes': N_CLASSES, 'epochs': NUM_EPOCHS,
           ...
           'val_acc': best['val'], 'val_best_epoch': best['epoch'],
           'test_acc': test['overall_acc'], 'test_balanced_acc': test['balanced_acc'],
    ...
    print(f'DONE - VAL {best["val"]:.3f} | TEST {test["overall_acc"]:.3f} (bal {test["balanced_acc"]:.3f}) | saved -> {out}')
