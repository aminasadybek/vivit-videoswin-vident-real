...
# Every data point is ANNOTATED WITH ITS VALUE. Curves match the metrics printed on the
# per-epoch line: train_acc(clip), val_acc(clip), val_acc(seg, reported), val_bal_acc(seg),
# train_loss(clip), val_loss(clip). Plus a combined comparison and a summary table/CSV.
#
...
def _annotate(xs, ys, color, dy, fmt='{:.3f}', fs=6.5):
    """Write each value next to its point, offset vertically so curves don't collide."""
...
def main():
    ...
    for fp in files:
        ...
        tr_loss   = [e.get('train_loss') for e in log]    # clip-level
        vl_loss   = [e.get('val_loss') for e in log]      # clip-level
        has_vloss = all(v is not None for v in vl_loss) and len(vl_loss) > 0
        best_ep   = d.get('val_best_epoch')

        # ============ Accuracy (4 curves, all values annotated) ============
    ...
    # ============ Combined val accuracy (segment, reported) ============
    if len(acc_curves) > 1:
        ...
        plt.title('Validation accuracy (segment, reported) - all runs')
    ...
    if summary:
        ...
        print('\n' + '  '.join(f'{h:<12}' for h in hdr)); print('-' * 130)
        best_i = max(range(len(summary)), key=lambda i: float(summary[i][2]))   # highest VAL(seg)
