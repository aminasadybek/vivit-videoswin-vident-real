# Vident-real teeth segmentation - ViViT-B only, full metric suite.
...
# NOTE: ViViT positional embeddings are fixed to 32 frames -> CLIP_LEN must stay 32.
#
# Install:  C:\ProgramData\Miniconda3\python.exe -m pip install peft transformers pynvml
# Run:      C:\ProgramData\Miniconda3\python.exe vident_vivit_seg_metrics.py
...
MODEL      = 'vivit'              # this file is ViViT-B only
REGIME     = 'lora'               # 'full' | 'lora'    <- run both
SMOKE_TEST = False                # 2 videos / 1 epoch sanity run; flip to False for the real run

CLIP_LEN   = 32                   # ViViT REQUIRES 32; Swin uses 32 too so efficiency metrics are comparable
IMG_SIZE   = 224
BATCH_SIZE = 4
NUM_EPOCHS = 15
...
class VivitSeg(nn.Module):
    ...
    def __init__(self):
        ...
        self.dec  = nn.ModuleList([up(768, 256), up(256, 128), up(128, 64), up(64, 32)])
        self.head = nn.Conv2d(32, 1, 1)
    def forward(self, x):
        x = x.permute(0, 2, 1, 3, 4).contiguous()                 # (B,T,C,H,W)
        tok = self.backbone(pixel_values=x).last_hidden_state[:, 1:, :]
        B, N, C = tok.shape
        Tt = N // (self.GRID * self.GRID)
...
def build_model(model_name, regime):
    ...
    if regime == 'full':
        ...
        cfg = LoraConfig(r=LORA_R, lora_alpha=LORA_ALPHA, lora_dropout=0.05,
                         ...
                         modules_to_save=['dec.0', 'dec.1', 'dec.2', 'dec.3', 'head'])
...
def main():
    ...
    results = {'model': 'vivit_b', 'regime': REGIME,
