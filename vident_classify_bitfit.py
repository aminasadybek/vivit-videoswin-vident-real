# Vident-real DENTAL QUADRANT classification - BitFit fine-tuning (bias terms + head only).
# A parameter-efficient FT technique: freezes all weights, trains only bias parameters.
# Same pipeline as the focal script (segments-as-videos, stride-2, class-weighted focal loss, all metrics);
# only the fine-tuning regime differs. Saves into the same focal folder so the spectrum plots together.
#
#   MODEL = 'swin'|'vivit'   -> BitFit only (bias terms + head). Run both models.
#
# Install:  C:\ProgramData\Miniconda3\python.exe -m pip install peft transformers pynvml openpyxl
# Run:      C:\ProgramData\Miniconda3\python.exe vident_classify_bitfit.py
...
SAVE_DIR    = r'C:\Users\ESAI406_student2\Vident_cls_results_bitfit_focal'
...
BATCH_SIZE = 4
NUM_EPOCHS = 15
LR         = 1e-3                 # BitFit trains only bias terms + head, so a higher LR is fine
...
# (LoRA config unused here; this script does BitFit only)
# Imbalance handling: class-weighted FOCAL loss (down-weights easy examples to focus on hard/minority
# cases). Keeps the inverse-frequency class weights AND adds the (1-p_t)^gamma focusing term.
FOCAL_GAMMA = 2.0                 # focusing strength (paper default; not finely tunable on 13 val segments)
...
def parse_labels(xlsx):
    ...
    ws = openpyxl.load_workbook(xlsx, data_only=True)['Modified']
...
def match_folders(split_dir, videos):
    """Map each hex folder -> its video (total frames + ranges) by matching folder frame count
    to the spreadsheet #frames (counts are distinct)."""
    ...
    used = [False] * len(videos); mapping = []
    print(f'  matching {len(folders)} folders to {len(videos)} videos in {os.path.basename(split_dir)}:')
    for f in folders:
        n = fcount[f]
        ...
        v = videos[best_j]; used[best_j] = True
        ...
        flag = '' if best_d == 0 else f'  <-- INEXACT (folder {n} vs label {v["total"]}, off {best_d})'
...
def build_model(model_name, regime=None):
    """BitFit: freeze all WEIGHTS, train only the BIAS terms across the network + the classification head."""
    if model_name == 'swin':
        net = swin3d_b(weights=Swin3D_B_Weights.KINETICS400_V1)
        net.head = nn.Linear(net.head.in_features, N_CLASSES)
        head, is_hf = net.head, False
    else:
        ...
        head, is_hf = net.classifier, True

    for p in net.parameters(): p.requires_grad = False        # freeze everything
    for p in head.parameters(): p.requires_grad = True        # always train the head

    # BitFit: train ONLY bias parameters of Linear/Conv layers (every '*.bias'), everything else frozen.
    # NOTE: normalization-layer biases (LayerNorm/BatchNorm) are EXCLUDED - torchvision's Video Swin
    # computes some norms functionally, and enabling grads on their bias breaks the backward pass
    # (NativeLayerNormBackward invalid-gradient error). Excluding them is also the standard BitFit variant.
    norm_bias_ids = set()
    for m in net.modules():
        if isinstance(m, (nn.LayerNorm, nn.BatchNorm1d, nn.BatchNorm2d, nn.BatchNorm3d, nn.GroupNorm)):
            if getattr(m, 'bias', None) is not None:
                norm_bias_ids.add(id(m.bias))
    n_bias = 0
    for nm, p in net.named_parameters():
        if nm.endswith('.bias') and id(p) not in norm_bias_ids:
            p.requires_grad = True; n_bias += 1
    print(f'bitfit: training {n_bias} (non-norm) bias tensors + head')

    tr = sum(p.numel() for p in net.parameters() if p.requires_grad)
    tot = sum(p.numel() for p in net.parameters())
    print(f'trainable params: {tr:,} / {tot:,} ({100*tr/max(tot,1):.3f}%)')
...
def main():
    ...
    print(f'{NAME} | regime=bitfit | smoke={SMOKE_TEST} | {N_CLASSES} classes (Q1-Q4)')
    ...
    def focal_loss(logits, target, weight, gamma=FOCAL_GAMMA):
        # class-weighted multi-class focal loss:
        #   FL = sum_i [ w_c * (1 - p_t)^gamma * (-log p_t) ] / sum_i w_c
        # (at gamma=0 this is exactly class-weighted cross-entropy; gamma>0 down-weights easy clips)
        logp = F.log_softmax(logits, dim=1)
        logp_t = logp.gather(1, target[:, None]).squeeze(1)       # log p_t
        p_t = logp_t.exp()
        wt = weight[target]                                       # per-sample class weight
        return (wt * (1 - p_t).pow(gamma) * (-logp_t)).sum() / wt.sum().clamp_min(1e-8)

    loss_fn = lambda out, y: focal_loss(out, y, w)
    print(f'loss = class-weighted FOCAL (gamma={FOCAL_GAMMA})')
    ...
    model = build_model(MODEL, 'bitfit')
    ...
    ckpt = os.path.join(SAVE_DIR, f'{MODEL}_bitfit_cls_focal_best.pth')
    ...
    SEP = '=' * 70
    print('\n' + SEP)
    print(f'  RESULTS: vident-real quadrant cls | {NAME} | bitfit | {NUM_EPOCHS}ep')
    print(SEP)
    rows = [
    ...
    res = {'model': MODEL, 'regime': 'bitfit', 'n_classes': N_CLASSES, 'epochs': NUM_EPOCHS,
           'loss_type': 'focal', 'focal_gamma': FOCAL_GAMMA,
    ...
    out = os.path.join(SAVE_DIR, f'results_cls_focal_{MODEL}_bitfit.json')
