# Vident-real DENTAL QUADRANT classification - FROZEN (linear probe) & PARTIAL (last-K blocks) FT.
# Extends the full/lora fine-tuning comparison with the low-capacity end of the spectrum.
...
#
#   MODEL = 'swin'|'vivit'   REGIME = 'frozen'|'partial'   PARTIAL_K = last blocks to unfreeze
#
# Install:  C:\ProgramData\Miniconda3\python.exe -m pip install peft transformers pynvml openpyxl
# Run:      C:\ProgramData\Miniconda3\python.exe vident_classify_ft.py
...
REGIME     = 'partial'             # 'frozen' (head only) | 'partial' (last-K blocks + head)
...
BATCH_SIZE = 4
NUM_EPOCHS = 15
PARTIAL_K  = 2                    # REGIME='partial': how many of the LAST transformer blocks to unfreeze
LR         = {'frozen': 1e-3, 'partial': 5e-4}[REGIME]   # head-only can take a higher LR than partial
...
# (LoRA config unused here; this script does frozen/partial only)
...
def build_clips(segments, stride):
    ...
    for seg_id, (folder, start, end, label) in enumerate(segments):
        ...
        seg = frames[s0:e0]
        print(f'seg_id {seg_id}: label {label}, range {start}-{end} -> using files {s0+1}..{e0} ({len(seg)} frames of {n} total)')
        m = len(seg)
        if m == 0: continue
...
def build_model(model_name, regime):
    """REGIME:
       'frozen'  -> freeze the whole backbone, train ONLY the classification head (linear probe).
       'partial' -> freeze the backbone, train the LAST PARTIAL_K transformer blocks + the head.
    """
    if model_name == 'swin':
    ...
    if regime == 'partial':
        # find transformer blocks in order, unfreeze the last K (+ the final norm so their output is calibrated)
        block_types = ('SwinTransformerBlock', 'VivitLayer')
        blocks = [m for _, m in net.named_modules() if type(m).__name__ in block_types]
        assert blocks, 'no transformer blocks found for partial FT'
        k = min(PARTIAL_K, len(blocks))
        for blk in blocks[-k:]:
            for p in blk.parameters(): p.requires_grad = True
        for nm, m in net.named_modules():                     # train the final layernorm if present
            if nm.endswith('layernorm') or nm == 'norm':
                for p in m.parameters(): p.requires_grad = True
        print(f'partial FT: unfroze last {k}/{len(blocks)} blocks + head')
    else:
        print('frozen FT (linear probe): training head only')
...
def main():
    ...
    ckpt = os.path.join(SAVE_DIR, f'{MODEL}_{REGIME}_cls_focal_best.pth')
    ...
    out = os.path.join(SAVE_DIR, f'results_cls_focal_{MODEL}_{REGIME}.json')
